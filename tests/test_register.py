...
def test_default_M_axis_map():
    """Verified Blender-extracted (cm) -> Disguise (m) map: dis = 0.01*(-T_y, T_z, T_x).
    It is a REFLECTION (det<0) on purpose -- it undoes the Blender FBX importer's Y-flip.
    See docs/ue-disguise-axis-mapping.md."""
    M = default_M()
    Rm = M[:3, :3]
    assert np.linalg.det(Rm) < 0                       # reflection (undoes Blender Y-flip)
    assert np.allclose(apply_M(M, np.array([[100.0, 0.0, 0.0]]))[0], [0, 0, 1], atol=1e-9)
    assert np.allclose(apply_M(M, np.array([[0.0, 100.0, 0.0]]))[0], [-1, 0, 0], atol=1e-9)
    assert np.allclose(apply_M(M, np.array([[0.0, 0.0, 100.0]]))[0], [0, 1, 0], atol=1e-9)
